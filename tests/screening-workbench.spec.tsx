...
describe('S3 screening workbench', () => {
  it('does not enter S3 after query completion and sends rules.propose only after explicit continue', async () => {
    ...
    expect(screen.getByText(zh['workbench.rules.waitingAgent'])).toBeTruthy()
    releaseSend?.()
  })
  ...
  it('requires applying an Agent structured suggestion before local editing and never persists local edits by itself', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: zh['workbench.rules.askAgent'] }))
    await waitFor(() => { expect(sendIntent).toHaveBeenCalledTimes(1) })
    expect(sendIntent.mock.calls[0]?.[0]).toMatchObject({ kind: 'rules.adjust', instruction: '扩展到拟建来源', rules: [{ name: '用户本地编辑' }] })
  ...
  it('marks previews stale after a local edit and sends preview/confirm only on explicit actions', async () => {
    ...
    await waitFor(() => { expect(sendIntent.mock.calls[0]?.[0]).toMatchObject({ kind: 'rules.confirm', previewArtifactId: previewRef.id }) })
    ...
    expect(screen.getByRole('button', { name: zh['workbench.rules.confirm'] }).hasAttribute('disabled')).toBe(true)
    fireEvent.click(screen.getByRole('button', { name: zh['workbench.rules.runPreview'] }))
    await waitFor(() => { expect(sendIntent.mock.calls[1]?.[0]).toMatchObject({ kind: 'rules.preview', rules: [{ name: '已修改名称' }] }) })
  ...
  it('renders classification totals, filters real classified rows, and traces source through stable decision without S4 results', async () => {
    ...
    expect(screen.getByText(zh['workbench.classification.s4Unavailable'])).toBeTruthy()
    ...
    expect(screen.getByText(/dataDisposition=normalized/u)).toBeTruthy()

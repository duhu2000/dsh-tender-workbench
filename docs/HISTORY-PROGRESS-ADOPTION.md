...
早期四产品脚本将入口统一视为 button，因 AI填表 0.2.27 实际使用公开的 `role=link / aria-label=AI填表 / href=/form-fill/` 而产生假阴性，并非产品入口缺失。已改为按各插件公开语义角色匹配并核验链接目标；以清洗 0.9.7 / 访前 0.1.22 / 填表 0.2.28 / 招投标候选 0.5.8 重跑 **通过**：三产品依次切入独立 Session，回切招投标，从 Profile 历史明确打开原 Session，原 projection 不变，随后真实重启历史恢复通过。后续组合业务回归仍须覆盖各产品完整任务历史、热卸载重装及真实 Provider 权限分支，不以入口验收代签。

...
export const TENDER_AGENT_SKILL: SkillRegistration = {
  ...
  content: [
    ...
    '- Agent 分析范围固定为当前分类中全部 `include + observe + manual-review`；`exclude + unmatched` 不进入分析。不得接收或生成 records/classifications 子范围和 batchSize。',
    '- 调用 `tender_workbench_analysis_next` 后，只引用该稳定批次返回的 `recordRef` / `evidenceRef`，再调用 `tender_workbench_analysis_commit`。commit 返回 `remaining > 0` 时，必须使用返回的 `projectionRevision` 继续 next → commit；仅在 `completed === eligibleTotal` 时结束本轮。不得维护隐藏游标、重做已提交批次或把部分结果称为完成。',
    ...
    '- `tender_workbench_generate_report` 只接受与当前 `contextFingerprint` 和 `createdAt`（作为 `contextAsOf` 原样传递）匹配的结构化 `ReportNarrativeV1`；Agent 不生成脚本、HTML/CSS、公式或版式。叙述可省略，Excel/PDF 仍由 Host 确定性生成。',

...
Before S6, the MVP now has two mandatory UI convergence iterations. S5.1 replaces the narrow query surface with the complete set of common, tender, and proposed-project conditions specified by the UX design and supported by the current qcc Schema, then aligns the query, overview, and data-detail work surfaces with that design. S5.2 aligns rules, preview, deterministic classification/trace, and optional Agent analysis with the same design. Existing Client structure is not a compatibility target: these pages may be refactored, and no simplified/advanced dual entry or legacy layout is retained.
...
- The screening view uses a criteria-list/editor workspace, keeps internal identifiers and revisions out of the business UI, emphasizes one primary action for the current state, and presents preview conclusions and conflicts before a bounded sample set.
...
- S5.1 and S5.2 are the mandatory design-convergence gates before S6. “Find opportunities” and “Screen candidates” must follow the checked-in HTML UX baseline; implementation cost or existing component structure is not grounds for a compatibility layout or an alternate page.
